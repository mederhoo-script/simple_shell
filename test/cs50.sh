...
main ()
{
  ...
  apt_config_url="https://packagecloud.io/install/repositories/cs50/repo/config_file.list?os=${os}&dist=${dist}&source=script"

  apt_source_path="/etc/apt/sources.list.d/cs50_repo.list"
  apt_keyrings_dir="/etc/apt/keyrings"
  if [ ! -d "$apt_keyrings_dir" ]; then
    mkdir -p "$apt_keyrings_dir"
  fi
  gpg_keyring_path="$apt_keyrings_dir/cs50_repo-archive-keyring.gpg"
    gpg_key_path_old="/etc/apt/trusted.gpg.d/cs50_repo.gpg"
